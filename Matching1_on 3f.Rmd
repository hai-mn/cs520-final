...
## Variable selection in PS

__Selecting the Covariates__  

  - Ideally, include covariates/variables $X$ that are related to (1) <span style="color:orange">treatment</span> and (2) outcomes    
      <span style="color:orange">• Continuous covariates are difficult to match, so categorize</span>    
      
  - Trade-off problem:    
      <span style="color:orange">• Too many covariates might be difficult to match and increase the variability</span>    
      <span style="color:orange">• Excluding important confounders might lead to biased results</span>     
      
  - Do not include variables that might be affected by treatment    
      <span style="color:orange">• Exclude from matching, but consider later in the outcome analysis</span>    

-- *Cody Chiuzan, PROPENSITY SCORE MATCHING A PRACTICAL TUTORIAL, March 19, 2018. Biostatistics, Epidemiology and Research Design (BERD) Lecture*   

<br />

  <ul> <li style="list-style-type: none;">One type of variable that should not be included in the matching process are those that may have been affected by the treatment of interest. This is especially important when the covariates, treatment indicator, and outcomes are all collected at the same point in time. If it is deemed to be critical to control for a variable potentially affected by treatment assignment, it is better to exclude that variable in the matching procedure and include it in the analysis model for the outcome.</li> </ul>

-- *Elizabeth A. Stuart, Matching methods for causal inference: A review and a look forward, 2010. Stat Sci.*

<br />

  <ul> <li style="list-style-type: none;">... suggest that variables that are unrelated to the exposure but related to the outcome should always be included in a PS model. The inclusion of these variables will increase the precision of the estimated exposure effect without increasing bias. In contrast, including variables that are related to the exposure but not the outcome will decrease the precision of the estimated exposure effect without decreasing bias.</li> </ul>   
  
-- *M. Alan Brookhart, Variable selection for propensity score models 2007. Am J Epidemiol*


## To-do

- Propensity Score Matching on 3 factors: `age`, `bmi` and `female`
...
hr.b <- hr.b %>% mutate(chdlabel = ifelse(chd == 1,
                       yes = 'Congenital Heart Disease',
                       no = 'Healthy Person'))
...
summary(lm(chronotropic.index~chd+loc+race.ethnicity, data = df.pm))
```