...
- Propensity Score Matching on 5 factors: `loc`, `age`, `bmi`, `female` and `race.ethnicity`
...
X<-hr.b[,c("loc","age","bmi","female","white","hispanic")]
...
fit <- glm(
  chd ~ as.factor(loc) + 
    as.factor(white) + as.factor(hispanic) + white.miss + hispanic.miss +
    female + 
    age + I(age^2) +
    bmi + I(bmi^2),
  family = binomial(),
...
```{r}
xvars <- c("loc","age","bmi","female","white","hispanic","ps")
